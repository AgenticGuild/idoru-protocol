...
contract PaymentGatewayV2 is Ownable, ReentrancyGuard {
    ...
    struct Quote {
        ...
        uint256 expiry;
        uint256 nonce;
        bytes32 pid; // computed id
    }

    bytes32 public constant EIP712_QUOTE_TYPEHASH = keccak256(
        "Quote(address payer,address payee,address token,uint256 amount,uint256 fee,uint256 expiry,uint256 nonce,bytes32 pid)"
    );
    ...
    uint256 public challengeWindow = 60; // seconds
    ...
    function computePid(
        ...
        return keccak256(abi.encode(address(this), block.chainid, payer, payee, token, amount, nonce, expiry));
    }

    function quote(Quote calldata q, bytes calldata facilitatorSig) external onlyFacilitator {
        // validate pid integrity
        if (q.pid != computePid(q.payer, q.payee, q.token, q.amount, q.nonce, q.expiry)) revert InvalidQuote();
    ...
    function refund(bytes32 pid) external onlyFacilitator nonReentrant {
        ...
        if (block.timestamp < p.expiry + challengeWindow) revert NotExpired();
        p.refunded = true;
